...
func newRouter(svc *gatewayService) *chi.Mux {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Route("/organization-iam", func(r chi.Router) {
			r.Group(func(r chi.Router) {
				r.Use(svc.authzWithOrganization)
				r.Get("/list-role", svc.listOrganizationRoleOrg_iamHandler)
				r.Get("/get-role", svc.getOrganizationRoleOrg_iamHandler)
				r.Get("/list-policy", svc.listOrganizationPolicyOrg_iamHandler)
				r.Get("/get-policy", svc.getOrganizationPolicyOrg_iamHandler)
				r.Get("/list-user-reserved", svc.listOrganizationUserReservedOrg_iamHandler)
				r.Get("/list-access-token", svc.listOrganizationAccessTokenOrg_iamHandler)
				r.Group(func(r chi.Router) {
					r.Use(middleware.AllowContentType(contenTypeJSON))
					r.Post("/put-role", svc.putOrganizationRoleOrg_iamHandler)
					r.Post("/put-policy", svc.putOrganizationPolicyOrg_iamHandler)
					r.Post("/attach-role", svc.attachOrganizationRoleOrg_iamHandler)
					r.Post("/attach-policy", svc.attachOrganizationPolicyOrg_iamHandler)
					r.Post("/detach-role", svc.detachOrganizationRoleOrg_iamHandler)
					r.Post("/detach-policy", svc.detachOrganizationPolicyOrg_iamHandler)
					r.Post("/delete-role", svc.deleteOrganizationRoleOrg_iamHandler)
					r.Post("/delete-policy", svc.deleteOrganizationPolicyOrg_iamHandler)
					r.Post("/delete-user-reserved", svc.deleteOrganizationUserReservedOrg_iamHandler)
					r.Post("/put-user-reserved", svc.putOrganizationUserReservedOrg_iamHandler)
					r.Post("/generate-access-token", svc.generateOrganizationAccessTokenOrg_iamHandler)
					r.Post("/update-access-token", svc.updateOrganizationAccessTokenOrg_iamHandler)
					r.Post("/delete-access-token", svc.deleteOrganizationAccessTokenOrg_iamHandler)
					r.Post("/attach-access-token-role", svc.attachOrganizationAccessTokenRoleOrg_iamHandler)
					r.Post("/detach-access-token-role", svc.detachOrganizationAccessTokenRoleOrg_iamHandler)
				})
			})
		})
		...
		r.Route("/code", func(r chi.Router) {
			...
			r.Group(func(r chi.Router) {
				...
				r.Group(func(r chi.Router) {
					...
					r.Post("/put-code-scan-setting", svc.putCodeScanSettingCodeHandler)
					r.Post("/delete-code-scan-setting", svc.deleteCodeScanSettingCodeHandler)
					...
					r.Post("/invoke-scan-code-scan", svc.invokeScanCodeScanCodeHandler)
				})
			})
		})
