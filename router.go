	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Route("/ai", func(r chi.Router) {
			...
			r.Group(func(r chi.Router) {
				...
				r.Group(func(r chi.Router) {
					r.Use(middleware.AllowContentType(contenTypeJSON))
					r.Post("/generate-remediation-proposal", svc.generateRemediationProposalDatasourceAIHandler)
				})
			})
		})
		...
		r.Route("/organization-iam", func(r chi.Router) {
			r.Use(svc.authzWithOrg)
			r.Get("/list-role", svc.listOrgRoleOrg_iamHandler)
			r.Get("/get-role", svc.getOrgRoleOrg_iamHandler)
			r.Get("/list-policy", svc.listOrgPolicyOrg_iamHandler)
			r.Get("/get-policy", svc.getOrgPolicyOrg_iamHandler)
			r.Get("/list-user-reserved", svc.listOrgUserReservedOrg_iamHandler)
			r.Get("/list-access-token", svc.listOrgAccessTokenOrg_iamHandler)
			r.Group(func(r chi.Router) {
				r.Use(middleware.AllowContentType(contenTypeJSON))
				r.Post("/put-role", svc.putOrgRoleOrg_iamHandler)
				r.Post("/put-policy", svc.putOrgPolicyOrg_iamHandler)
				r.Post("/attach-role", svc.attachOrgRoleOrg_iamHandler)
				r.Post("/attach-policy", svc.attachOrgPolicyOrg_iamHandler)
				r.Post("/detach-role", svc.detachOrgRoleOrg_iamHandler)
				r.Post("/detach-policy", svc.detachOrgPolicyOrg_iamHandler)
				r.Post("/delete-role", svc.deleteOrgRoleOrg_iamHandler)
				r.Post("/delete-policy", svc.deleteOrgPolicyOrg_iamHandler)
				r.Post("/delete-user-reserved", svc.deleteOrgUserReservedOrg_iamHandler)
				r.Post("/put-user-reserved", svc.putOrgUserReservedOrg_iamHandler)
				r.Post("/generate-access-token", svc.generateOrgAccessTokenOrg_iamHandler)
				r.Post("/update-access-token", svc.updateOrgAccessTokenOrg_iamHandler)
				r.Post("/delete-access-token", svc.deleteOrgAccessTokenOrg_iamHandler)
				r.Post("/attach-access-token-role", svc.attachOrgAccessTokenRoleOrg_iamHandler)
				r.Post("/detach-access-token-role", svc.detachOrgAccessTokenRoleOrg_iamHandler)
			})
		})
