...
func (g *gatewayService) createProjectWithOrganizationHandler(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	req := &organization.CreateProjectWithOrganizationRequest{}
	req.UserId = user.userID // force update by own userID
	if err := bind(req, r); err != nil {
		appLogger.Warnf(ctx, "Failed to bind request, req=%s, err=%+v", "CreateProjectWithOrganizationRequest", err)
	}
	if err := req.Validate(); err != nil {
		writeResponse(ctx, w, http.StatusBadRequest, map[string]interface{}{errorJSONKey: err.Error()})
		return
	}
	resp, err := g.organizationClient.CreateProjectWithOrganization(ctx, req)
	if err != nil {
